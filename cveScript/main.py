...
# Teams webhook URL
teams_webhook_url = "https://zalaris.webhook.office.com/webhookb2/5efa612b-bdd7-4ee3-bcca-ba9489a002e4@a16eb8e2-803d-4f22-849c-f3f335a60a39/IncomingWebhook/6d55e016a0024ec6bf55d93beb170719/f9164535-24c6-4bdf-924b-c341cbf08ede/V2UrVWLUasznvmgTPo2QQhz56p_pIvOQqEnvDV6XG7p4w1"
...
# Generate HTML table for new alerts
def generate_html_table(dataframe):
    html = '<table border="1">'
    html += '<tr>' + ''.join([f'<th>{col}</th>' for col in dataframe.columns]) + '</tr>'
    for _, row in dataframe.iterrows():
        html += '<tr>' + ''.join([f'<td>{cell}</td>' for cell in row]) + '</tr>'
    html += '</table>'
    return html

# Send HTML content to Microsoft Teams webhook
def send_to_teams(webhook_url, html_content):
    headers = {
...
# Split new alerts into chunks of 10 and send to Teams
chunk_size = 10
for i in range(0, len(new_alerts_df), chunk_size):
    chunk_df = new_alerts_df.iloc[i:i + chunk_size]
    html_table = generate_html_table(chunk_df)
    send_to_teams(teams_webhook_url, html_table)
