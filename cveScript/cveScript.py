...
for item in query_items:
    ...
    for row in rows:
        ...
        cursor.execute('''
            INSERT OR IGNORE INTO vulnerabilities (cve_id, published_date, cvss_v3_score, cvss_v2_score)
            VALUES (?, ?, ?, ?)
        ''', (cve_id, published_date, cvss_v3_score, cvss_v2_score))
...
# Fetch new alerts by comparing with the previous data
cursor.execute('''
    SELECT * FROM vulnerabilities
    WHERE cve_id NOT IN (
        SELECT cve_id FROM vulnerabilities WHERE published_date < DATE('now')
    )
''')
...
if new_alerts:
    new_data = [[alert[0], alert[1], alert[2], alert[3]] for alert in new_alerts]
    pdf.table(header, new_data)
else:
