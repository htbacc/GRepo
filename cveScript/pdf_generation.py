...
class PDF(FPDF):
    ...
    def table(self, header, data):
        col_widths = [40, 80, 40, 40]  # Adjusted column widths to match the number of headers
        col_widths += [40] * (len(header) - len(col_widths))  # Extend col_widths if there are more headers
